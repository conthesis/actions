...
from actions.deps import entity_fetcher, http_client
...
async def resolve_properties(
    entity_fetcher: EntityFetcher, properties: List[ActionProperty]
) -> Dict[str, Any]:
    return {p.name: await resolve_value(entity_fetcher, p) for p in properties}


async def perform_action(
    http_client, kind: str, properties: Dict[str, any]
) -> Dict[str, Any]:
    if kind == "identity":
